...
class FastCosyVoice2:
    ...
    def __init__(self, cosyvoice_model, trtllm_engine_dir, trtllm_tokenizer_dir, spk2info_path=None):
        ...
        # 🔴 新增：存储预计算的 Flow Prompt Cache（优化方案 B）
        self.flow_prompt_cache = {}  # spk_id -> cache dict
    ...
    def _inference_single_segment(self, text, prompt_text, prompt_speech_16k, zero_shot_spk_id='', stream=True, request_start_time=None, is_first_segment=True):
        ...
        try:
            ...
            # 🔴 优化方案 B：检查是否有预计算的 Flow Cache
            flow_cache = self.flow_prompt_cache.get(zero_shot_spk_id)
            use_flow_cache = flow_cache is not None
                
            context_load_time = (time.perf_counter() - context_start) * 1000
            if is_first_segment:
                cache_status = "✅启用Cache" if use_flow_cache else "❌未Cache"
                logging.info(f"[延迟分析-01] 上下文加载: {context_load_time:.2f}ms (spk_info检索+数据解析) {cache_status}")
            ...
            # 关键：prompt_token_pad 必须基于标净 hop_len 计算，不能改变
            ...
            for current_tokens, is_final in self._trtllm_generate_streaming(input_ids):
                ...
                while True:
                    ...
                    if tokens_needed <= len(speech_tokens):
                        ...
                        # 🔴 优化方案 B：使用带 Cache 的 token2wav
                        if use_flow_cache and token_offset == 0:
                            # 首块使用优化版 token2wav（利用预计算的 Cache）
                            tts_speech = self._token2wav_with_cache(
                                token=this_tts_speech_token,
                                cache_dict=flow_cache,
                            ...
                            if is_first_segment and not first_chunk_generated:
                                logging.info("[Flow Cache] ✅ 首块使用了预计算 Cache，跳过 prompt encoder 计算")
                        else:
                            # 后续块或未启用 Cache，使用原始 token2wav
                        ...
                        if not first_chunk_generated:
                            ...
                            if is_first_segment:
                                ...
                                logging.info(f"  └─ 音频合成(token2wav): {chunk_time:.2f}ms (step 5) {'[Cache优化]' if use_flow_cache else ''}")
    ...
    def _token2wav_with_cache(self, token, cache_dict, token_offset, uuid, stream=False, finalize=False, speed=1.0):
        """使用预计算 Cache 的优化 token2wav（仅首块有效）
        
        Args:
            token: 生成的 speech tokens (prompt + generated)
            cache_dict: 预计算的 Flow Prompt Cache
            token_offset: 当前 token 偏移量（应为 0）
            uuid: 请求 UUID
            stream: 是否流式模式
            finalize: 是否为最后一块
            speed: 语速
        ...
        # 从 Cache 中获取预计算的数据
        prompt_encoded_proj = cache_dict['prompt_encoded_proj'].to(self.device)
        prompt_speech_feat = cache_dict['prompt_speech_feat'].to(self.device)
        flow_embedding_proj = cache_dict['flow_embedding_proj'].to(self.device)
        flow_prompt_speech_token = cache_dict['flow_prompt_speech_token'].to(self.device)
        prompt_mel_len = cache_dict['prompt_mel_len']
        
        with torch.cuda.amp.autocast(model.fp16):
            # 步骤 1: Token Embedding（对整个生成 token 序列）
            token_len = token.shape[1]
            token_mask = torch.ones([1, token_len, 1], device=self.device).to(prompt_encoded_proj.dtype)
            token_embedded = model.flow.input_embedding(torch.clamp(token.to(self.device), min=0)) * token_mask
            
            # 步骤 2: Encoder（仅对生成部分；这里的 token 全是生成部分）
            generated_token_embedded = token_embedded  # 全部 token 都是生成部分
            generated_token_len = token_len

            # 特殊情况：token 很短或 finalize=True 时，直接走无前瞻分支
            if finalize is True or generated_token_len <= model.flow.pre_lookahead_len:
                generated_encoded, _ = model.flow.encoder(
                    generated_token_embedded,
                    torch.tensor([generated_token_len], dtype=torch.int32).to(self.device),
                    streaming=stream
                )
            else:
                # 带前瞻的 encoder（与 flow.inference 的逻辑一致）
                generated_context = generated_token_embedded[:, -model.flow.pre_lookahead_len:, :]
                generated_token_no_lookahead = generated_token_embedded[:, :-model.flow.pre_lookahead_len, :]
                generated_encoded, _ = model.flow.encoder(
                    generated_token_no_lookahead,
                    torch.tensor([generated_token_len], dtype=torch.int32).to(self.device),
                    context=generated_context,
                    streaming=stream
                )
            
            generated_encoded_proj = model.flow.encoder_proj(generated_encoded)
            
            # 步骤 3: 拼接 prompt 和 generated 的 encoder 输出（利用预计算的 prompt_encoded_proj）
            h = torch.cat([prompt_encoded_proj, generated_encoded_proj], dim=1)
            mel_len2 = h.shape[1] - prompt_mel_len
            
            # 步骤 4: 准备 Conditions
            conds = torch.zeros([1, prompt_mel_len + mel_len2, model.flow.output_size], 
                                device=self.device).to(h.dtype)
            conds[:, :prompt_mel_len] = prompt_speech_feat
            conds = conds.transpose(1, 2)
            
            # 步骤 5: Decoder
            mask = torch.ones([1, 1, prompt_mel_len + mel_len2], device=self.device).to(h.dtype)
            feat, _ = model.flow.decoder(
                mu=h.transpose(1, 2).contiguous(),
                mask=mask,
                spks=flow_embedding_proj,
                cond=conds,
                n_timesteps=10,
                streaming=stream
            )
            ...
            # 只取生成部分的 mel
            tts_mel = feat[:, :, prompt_mel_len:]
        
        # 步骤 6: 处理 mel offset
        tts_mel = tts_mel[:, :, token_offset * model.flow.token_mel_ratio:]
        
        # 步骤 7: HiFiGAN（与原始 token2wav 一致）
        if model.hift_cache_dict[uuid] is not None:
            hift_cache_mel, hift_cache_source = model.hift_cache_dict[uuid]['mel'], model.hift_cache_dict[uuid]['source']
            tts_mel = torch.concat([hift_cache_mel, tts_mel], dim=2)
        else:
            hift_cache_source = torch.zeros(1, 1, 0)
        
        if finalize is False:
            tts_speech, tts_source = model.hift.inference(speech_feat=tts_mel, cache_source=hift_cache_source)
            if model.hift_cache_dict[uuid] is not None:
                from cosyvoice.utils.common import fade_in_out
                tts_speech = fade_in_out(tts_speech, model.hift_cache_dict[uuid]['speech'], model.speech_window)
            model.hift_cache_dict[uuid] = {
                'mel': tts_mel[:, :, -model.mel_cache_len:],
                'source': tts_source[:, :, -model.source_cache_len:],
                'speech': tts_speech[:, -model.source_cache_len:]
            }
            tts_speech = tts_speech[:, :-model.source_cache_len]
        else:
            if speed != 1.0:
                assert model.hift_cache_dict[uuid] is None, 'speed change only support non-stream inference mode'
    ...
    def add_zero_shot_spk(self, prompt_text, prompt_speech_16k, zero_shot_spk_id):
        """添加零样本说话人（带 Flow Prompt Cache 预计算）"""
        # 保存原始的 prompt_text 字符串（用于 TensorRT-LLM）
        self.spk_prompt_text_raw[zero_shot_spk_id] = prompt_text
        
        # 调用原始方法添加说话人
        result = self.cosyvoice.add_zero_shot_spk(prompt_text, prompt_speech_16k, zero_shot_spk_id)
        
        # 🔴 优化方案 B：预计算 Flow Prompt Cache
        try:
            logging.info(f"[Flow Cache] 正在为说话人 {zero_shot_spk_id} 预计算 Flow Prompt Cache...")
            cache_start = time.perf_counter()
            
            # 获取 spk_info
            spk_info = self.cosyvoice.frontend.spk2info.get(zero_shot_spk_id)
            if spk_info is None:
                logging.warning(f"[Flow Cache] 说话人 {zero_shot_spk_id} 不存在，跳过 Cache 预计算")
                return result
            ...
            # 检查 flow 模块是否有 setup_cache 方法（类似 Triton 实现）
            model = self.cosyvoice.model
            if hasattr(model.flow, 'setup_cache'):
                # 使用 Triton 风格的 setup_cache（需要在 flow.py 中实现）
                # 这里先准备参数，实际调用需要 flow 模块支持
                logging.info(f"[Flow Cache] 检测到 flow.setup_cache 方法，使用 Triton 风格缓存")
                
                # 模拟 Triton 的预计算逻辑（添加 3 个前瞻 token）
                prompt_tokens_with_lookahead = torch.cat([
                    flow_prompt_speech_token,
                    flow_prompt_speech_token[:, :3]  # 添加前 3 个 token 作为前瞻
                ], dim=1)
                
                cache = model.flow.setup_cache(
                    prompt_tokens_with_lookahead.to(self.device),
                    prompt_speech_feat.to(self.device),
                    flow_embedding.to(self.device),
                    n_timesteps=10
                )
                
                # 深拷贝 cache（避免原地修改）
                self.flow_prompt_cache[zero_shot_spk_id] = {k: v.clone() for k, v in cache.items()}
                
            else:
                # 回退方案：预计算 Flow Encoder 部分（即使没有 setup_cache 也能优化一部分）
                logging.info(f"[Flow Cache] flow 模块无 setup_cache，使用 Encoder 预计算方案")
                
                with torch.cuda.amp.autocast(self.cosyvoice.model.fp16):
                    # 预计算 prompt 的 encoder 输出（这部分在每次推理时是固定的）
                    prompt_feat_encoded = model.flow.encoder(
                        prompt_speech_feat.to(self.device),
                        torch.tensor([prompt_speech_feat.shape[1]], dtype=torch.int32).to(self.device)
                    )
                    
                self.flow_prompt_cache[zero_shot_spk_id] = {
                    'prompt_feat_encoded': prompt_feat_encoded.clone(),
                    'prompt_speech_feat': prompt_speech_feat.clone(),
                    'flow_embedding': flow_embedding.clone(),
                    'flow_prompt_speech_token': flow_prompt_speech_token.clone(),
                }
            
            cache_time = (time.perf_counter() - cache_start) * 1000
            logging.info(f"[Flow Cache] ✅ 说话人 {zero_shot_spk_id} 的 Flow Cache 预计算完成，耗时: {cache_time:.2f}ms")
            
        except Exception as e:
            logging.warning(f"[Flow Cache] 预计算失败: {e}，将在推理时实时计算")
            import traceback
            traceback.print_exc()
    ...
    def save_spkinfo(self):
        """保存说话人信息到指定路径（包含 Flow Prompt Cache）"""
        # 🔴 保存前先将 Flow Cache 同步到 spk2info
        for spk_id, cache_dict in self.flow_prompt_cache.items():
            if spk_id in self.cosyvoice.frontend.spk2info:
                self.cosyvoice.frontend.spk2info[spk_id]['flow_prompt_cache'] = cache_dict
        
        if self.spk2info_path:
            torch.save(self.cosyvoice.frontend.spk2info, self.spk2info_path)
            logging.info(f"说话人信息已保存到: {self.spk2info_path} (包含 {len(self.flow_prompt_cache)} 个 Flow Cache)")
        else:
            return self.cosyvoice.save_spkinfo()
    
    def load_spk_prompt_text_raw(self, spk_prompt_text_raw_dict):
        """从外部加载原始 prompt_text 映射（并预计算/加载 Flow Cache）"""
        self.spk_prompt_text_raw.update(spk_prompt_text_raw_dict)
        
        # 🔴 优化方案 B：为已加载的 spk_id 预计算或加载 Flow Cache
        logging.info(f"[Flow Cache] 开始为 {len(spk_prompt_text_raw_dict)} 个说话人加载/预计算 Flow Prompt Cache...")
        cache_loaded_count = 0
        cache_computed_count = 0
        cache_needs_save = False
        
        for spk_id in spk_prompt_text_raw_dict.keys():
            if spk_id in self.flow_prompt_cache:
                logging.info(f"[Flow Cache] 说话人 {spk_id} 的 Cache 已在内存中，跳过")
                continue
            
            try:
                # 获取 spk_info
                spk_info = self.cosyvoice.frontend.spk2info.get(spk_id)
                if spk_info is None:
                    logging.warning(f"[Flow Cache] 说话人 {spk_id} 不存在，跳过")
                    continue
                
                # 🔴 第一步：尝试从 spk2info 加载已保存的 Cache
                if 'flow_prompt_cache' in spk_info:
                    cached_data = spk_info['flow_prompt_cache']
                    # 验证缓存数据完整性
                    required_keys = {'prompt_encoded_proj', 'prompt_speech_feat', 'flow_embedding_proj', 
                                     'flow_prompt_speech_token', 'prompt_mel_len'}
                    if all(k in cached_data for k in required_keys):
                        self.flow_prompt_cache[spk_id] = cached_data
                        cache_loaded_count += 1
                        logging.info(f"[Flow Cache] ✅ {spk_id} 从 spk2info 加载 Cache 成功")
                        continue
                    else:
                        logging.warning(f"[Flow Cache] {spk_id} 的 Cache 数据不完整，重新计算")
                ...
                # 预计算 prompt 的 encoder 输出
                model = self.cosyvoice.model
                with torch.cuda.amp.autocast(model.fp16):
                    # 预计算 prompt token 的 embedding + encoder
                    prompt_token_embedded = model.flow.input_embedding(
                        torch.clamp(flow_prompt_speech_token.to(self.device), min=0)
                    )
                    prompt_encoded, _ = model.flow.encoder(
                        prompt_token_embedded,
                        torch.tensor([flow_prompt_speech_token.shape[1]], dtype=torch.int32).to(self.device),
                        streaming=False
                    )
                    prompt_encoded_proj = model.flow.encoder_proj(prompt_encoded)
                    
                    # 预计算 embedding 投影
                    flow_embedding_norm = torch.nn.functional.normalize(flow_embedding.to(self.device), dim=1)
                    flow_embedding_proj = model.flow.spk_embed_affine_layer(flow_embedding_norm)
                
                cache_dict = {
                    'prompt_encoded_proj': prompt_encoded_proj.clone().cpu(),  # 保存到 CPU 避免占用 GPU 显存
                    'prompt_speech_feat': prompt_speech_feat.clone(),
                    'flow_embedding_proj': flow_embedding_proj.clone().cpu(),
                    'flow_prompt_speech_token': flow_prompt_speech_token.clone(),
                    'prompt_mel_len': prompt_speech_feat.shape[1],
                }
                
                self.flow_prompt_cache[spk_id] = cache_dict
                # 同步到 spk2info（下次启动可直接加载）
                spk_info['flow_prompt_cache'] = cache_dict
                cache_needs_save = True
                cache_computed_count += 1
                
                cache_time = (time.perf_counter() - cache_start) * 1000
                logging.info(f"[Flow Cache] ✅ {spk_id} Cache 预计算完成，耗时: {cache_time:.2f}ms")
                
            except Exception as e:
                logging.warning(f"[Flow Cache] 说话人 {spk_id} Cache 加载/预计算失败: {e}")
                import traceback
                traceback.print_exc()
        
        # 🔴 第三步：如果有新计算的 Cache，自动保存到 spk2info.pt
        if cache_needs_save and self.spk2info_path:
            try:
                torch.save(self.cosyvoice.frontend.spk2info, self.spk2info_path)
                logging.info(f"[Flow Cache] 💾 已将新计算的 Cache 保存到 {self.spk2info_path}")
            except Exception as e:
                logging.warning(f"[Flow Cache] 保存失败: {e}")
        
        logging.info(f"[Flow Cache] 加载完成: {cache_loaded_count} 个从磁盘加载, {cache_computed_count} 个新计算")
...
if __name__ == "__main__":
    ...
    if available_spks:
        ...
        for _ in range(2):
            for _ in fast_cosyvoice.inference_zero_shot(
                "哼，算你有良心。拿来我看看！", "", None, zero_shot_spk_id=warmup_speaker, stream=True):
                pass
            for _ in fast_cosyvoice.inference_zero_shot(
                "你这个玩笑一点都不好笑！吓死我了... ", "", None, zero_shot_spk_id=warmup_speaker, stream=True):
                pass
    else:
        print("未找到可用说话人，跳过预热")
