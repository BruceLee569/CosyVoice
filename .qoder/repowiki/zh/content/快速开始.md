...
- pretrained_models/Fun-CosyVoice3-0.5B/cosyvoice3.yaml：CosyVoice3 模型配置示例；
...
- 配置文件：cosyvoice3.yaml 等，定义采样率、模型结构、前端处理等。
...
- 关键点：
  ...
  - inference_sft、inference_zero_shot、inference_cross_lingual、inference_instruct2 等方法；
...
Mode --> |自然语言控制| INSTR["调用 inference_instruct2"]
SFT --> Save["保存音频文件"]
ZS --> Save
CL --> Save
INSTR --> Save
...
- 自然语言控制（CosyVoice3）
  - 方法：inference_instruct2(tts_text, instruct_text, prompt_wav, zero_shot_spk_id='', stream=False, speed=1.0, text_frontend=True)
  - 适用场景：通过 instruct 文本控制情感、语速、方言等；
  - 关键参数：
    - tts_text：目标合成文本；
    - instruct_text：控制指令；
    - prompt_wav：参考音频（用于音色复刻）；
    - zero_shot_spk_id：可选的零样本音色 ID；
    - stream/speed/text_frontend：同上。
- 语音转换（VC）
...
**章节来源**
- [cosyvoice/cli/cosyvoice.py](file://cosyvoice/cli/cosyvoice.py#L80-L137)
- [cosyvoice/cli/cosyvoice.py](file://cosyvoice/cli/cosyvoice.py#L176-L185)

### 模型配置与采样率
- CosyVoice3 的配置文件示例展示了采样率、token 帧率、流式推理参数等关键配置项；
- 采样率在配置文件中定义，推理时会据此保存音频。

**章节来源**
- [pretrained_models/Fun-CosyVoice3-0.5B/cosyvoice3.yaml](file://pretrained_models/Fun-CosyVoice3-0.5B/cosyvoice3.yaml#L8-L14)
...
- 启动 Web 界面
  - 命令：python webui.py --port 50000 --model_dir pretrained_models/Fun-CosyVoice3-0.5B
  - 说明：启动 Gradio 界面，监听 0.0.0.0:50000；--model_dir 指定模型目录或仓库 ID。
- 启动 FastAPI 服务
  - 命令：python server.py --port 50000 --model_dir pretrained_models/Fun-CosyVoice3-0.5B
